...
pipeline {
  ...
  stages {
    ...
    stage('Version project') {
      ...
      steps {
        script {
          build job: 'operations/create-a-release', parameters: [string(name: 'Project', value: 'inbound-outbound'), string(name: 'Type', value: params.VersionType)]
          gitVersion = sh(
            script: '''
              JQ=./jq
              curl https://stedolan.github.io/jq/download/linux64/jq > $JQ && chmod +x $JQ
              curl -H "Accept: application/vnd.github.manifold-preview" -H 'Authorization: token '$GH_TOKEN'' -s 'https://api.github.com/repos/cjww-development/inbound-outbound/releases/latest' | ./jq -r '.tag_name'
            ''',
            returnStdout: true
          ).trim()
        }
